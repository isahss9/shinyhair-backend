    ...
    try {
        ...
        for (let post of posts) {
            const [curtidas] = await db.execute(
                `SELECT COUNT(*) total FROM curtidas_posts WHERE post_id=?`,
                [post.id]
            );
            post.curtidas = curtidas[0].total;
            const [respostas] = await db.execute(`
                SELECT respostas.*, usuarios.nome
                FROM respostas
...
app.post("/api/respostas", async (req, res) => {
    ...
    try {
        await db.execute(
            `INSERT INTO respostas (post_id, usuario_id, conteudo) VALUES (?, ?, ?)`,
            [post_id, req.session.usuario.id, conteudo]
        );
        res.json({ sucesso: true });
    } catch (err) {
        console.error(err);
...
app.delete("/api/posts/:id", async (req, res) => {
    ...
    try {
        ...
        const post = rows[0];
        const admin = req.session.usuario.email === ADMIN_EMAIL;
        const dono = req.session.usuario.id === post.usuario_id;
        if (!admin && !dono) {
...
app.delete("/api/respostas/:id", async (req, res) => {
    ...
    try {
        ...
        const admin = req.session.usuario.email === ADMIN_EMAIL;
        const dono = req.session.usuario.id === resposta.usuario_id;
        if (!admin && !dono) {
            return res.status(403).json({ mensagem: "Sem permissão" });
        }
...
app.post("/api/posts/:id/curtir", async (req, res) => {
    ...
    try {
        ...
        const [curtida] = await db.execute(
            `SELECT * FROM curtidas_posts WHERE post_id=? AND usuario_id=?`,
            [postId, usuarioId]
        );
        if (curtida.length > 0) {
            await db.execute(
                `DELETE FROM curtidas_posts WHERE post_id=? AND usuario_id=?`,
                [postId, usuarioId]
            );
        } else {
            await db.execute(
                `INSERT INTO curtidas_posts (post_id, usuario_id) VALUES (?, ?)`,
                [postId, usuarioId]
            );
        }
        res.json({ sucesso: true });
